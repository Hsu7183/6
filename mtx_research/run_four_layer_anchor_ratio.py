...
from mtx_research.anchor_body_bins import (
    BIN_MODE_ANCHOR_RATIO,
    EXPECTED_COMBOS,
    RATIO_REFERENCE_ANCHOR,
    scan,
)
...
def _scan_or_reuse(
    ...
    if skip_existing and summary.exists() and by_year.exists() and daily.exists() and trades.exists() and html.exists():
        print(f"[{layer.label}] reuse existing anchor-ratio matrix: {outdir}")
    ...
    print("=" * 72)
    print(f"[{layer.label}] start anchor-ratio matrix")
    print(f"data={data_path}")
    print(f"session={session.key} ({session.label})")
    print(f"reference_anchor={reference_anchor:,.0f}")
    print(
        "cost="
    ...
    return scan(
        ...
        bin_mode=BIN_MODE_ANCHOR_RATIO,
        reference_anchor=reference_anchor,
    )


def _build_layer_threshold(layer: Layer, paths: dict[str, Path], outdir: Path, reference_anchor: float) -> dict[str, Path]:
    print(f"[{layer.label}] build anchor-ratio threshold report")
    return build_report(
        paths["summary"],
        paths["by_year"],
        outdir,
        cost=cost_for_instrument(layer.instrument),
        layer_label=f"{layer.label} 比例",
        title=f"{layer.label} 定錨比例勝率門檻總表",
        description=(
            f"{layer.label}：{EXPECTED_COMBOS:,} 組定錨比例參數矩陣。"
            f"前K實體與 OpenGap 由原點數區間除以 {reference_anchor:,.0f} 換算成比例，"
            "再用每筆訊號自己的 Anchor 做比例判斷；Penetrate 固定 1 點。"
        ),
...
def _write_index(root: Path, layer_outputs: list[tuple[Layer, Path, Path]], reference_anchor: float) -> Path:
    rows = []
    ...
    html = root / "four_layer_anchor_ratio_index.html"
    html.write_text(
        f"""<!doctype html>
<html lang="zh-Hant">
<head>
<meta charset="utf-8">
<title>四層定錨比例策略矩陣索引</title>
<style>
...
<header>
<h1>四層定錨比例策略矩陣索引</h1>
<div class="sub">前K實體與 OpenGap 用比例區間判斷；比例區間由原點數除以 {reference_anchor:,.0f} 換算。小台日盤、小台全日、大台日盤、大台全日各自產生矩陣與門檻總表。</div>
</header>
<main>
<p><a href="four_layer_anchor_ratio_threshold_report.html">合併四層定錨比例勝率門檻總表</a></p>
<table>
...
def run_all(
    ...
    *,
    ...
    for layer in selected_layers:
        ...
        matrix_paths = _scan_or_reuse(layer, layer_outdir, progress_every, skip_existing, reference_anchor)
        threshold_paths = _build_layer_threshold(layer, matrix_paths, layer_outdir, reference_anchor)
    ...
    combined_html = outdir / "four_layer_anchor_ratio_threshold_report.html"
    ...
    write_html(
        ...
        description=(
            ...
            f"每層各自掃描 {EXPECTED_COMBOS:,} 組定錨比例參數；"
            f"前K實體與 OpenGap 由原點數除以 {reference_anchor:,.0f} 換算成比例。"
        ),
        matrix_href=None,
        daily=daily,
        trades=trades,
    )
    index_html = _write_index(outdir, layer_links, reference_anchor)
    return {
        "index": index_html,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run four-layer anchor-ratio strategy matrix and reports.")
    parser.add_argument(
        "--outdir",
        type=Path,
        default=Path("report_outputs") / "four_layer_anchor_ratio_20000",
    )
    parser.add_argument("--only", nargs="*", choices=[layer.key for layer in LAYERS])
    parser.add_argument("--progress-every", type=int, default=100_000)
    parser.add_argument("--skip-existing", action="store_true")
    parser.add_argument("--reference-anchor", type=float, default=RATIO_REFERENCE_ANCHOR)
